...
void EnsCoord::supprime(const Coord &c) {
  ...
  if (pos != -1) {
    ...
    throw std::invalid_argument("Coordonnée " + std::to_string(c.getLig()) +
                                ", " + std::to_string(c.getCol()) +
...
EnsCoord voisines(const Coord &c) {
  EnsCoord ens({});
  for (int i = std::max(c.getLig() - 1, 0);
       i <= std::min(c.getLig() + 1, TAILLE_GRILLE - 1); i++) {
    for (int j = std::max(c.getCol() - 1, 0);
         j <= std::min(c.getCol() + 1, TAILLE_GRILLE - 1); j++) {
      if (i != c.getLig() || j != c.getCol()) {
